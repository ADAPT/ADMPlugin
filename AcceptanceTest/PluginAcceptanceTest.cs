...
namespace AgGateway.ADAPT.AcceptanceTest
{
  ...
  public class PluginAcceptanceTest
  {
    ...
    {
      _cardPath = DatacardUtility.WriteDatacard("ADMSetup_V1_0_8");
    ...
    {
      _cardPath = DatacardUtility.WriteDatacard("ADMDoc_V1_0_8");

      var plugin = new Plugin();
      var applicationDataModel = plugin.Import(_cardPath);

      Assert.AreNotEqual(0, applicationDataModel.First().Catalog.Connectors.First().DeviceElementConfigurationId);
    }

    [Test]
    public void GivenFlagSessionsLayerCardWhenRoundTripThenModelAreSame()
    {
      _cardPath = DatacardUtility.WriteDatacard("FlagSessionLayers_V1");
